...
| `getHistory(q?)` | `SymbolParams?` | `Promise<Order[]>` *(absent : HL ; HL l'expose via `dex.native.account().getHistoricalOrders()`)* |
...
| `edit(i)` | `EditOrderParams` | `Promise<{ name: string; id: string }>` *(identité du nouvel ordre seulement — pas un snapshot complet comme `place()`)* |
...
Routes **Hyperliquid** : `wallet ↔ wallet` (perp↔spot, `usdClassTransfer`) · `to: { subAccount }`
(`subAccountTransfer`, `spot` si `asset`) · `to: { account }` (`usdSend` / `spotSend` si `asset`).

```ts
await dex.transfers().transfer({ from: { wallet: 'perp' }, to: { wallet: 'spot' }, amount: '10' });
await dex.transfers().transfer({ to: { subAccount: '0x…' }, amount: '10' });
await dex.transfers().transfer({ to: { account: '0x…' }, asset: 'USDC', amount: '10' });
```
